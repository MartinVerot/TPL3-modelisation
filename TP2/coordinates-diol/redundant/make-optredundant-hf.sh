...
if ! [[ -f head-optredundant-hf &&  -f  bottom ]]; then
...
echo "%chk=$NAME-opt" > $NAME-redundant-hf-opt.com
cat head-optredundant-hf $NAME.cut $NAME-bottom >> $NAME-redundant-hf-opt.com
rm $NAME.cut
